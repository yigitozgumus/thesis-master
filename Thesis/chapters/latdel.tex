...
stabilize the training of encoder network present in the overall framework. Section \ref{sec:fanogan} and 
\ref{sec:ebgan} presents some of these improvements that are later adopted by our framework. Their contribution
...
stabilization issues of the adversarial training, and inference stage of the framework that
...
inference time. F-AnoGAN (Fast AnoGAN ) framework \cite{pub.1111824956} aims to eliminate the inference computation time 
...
The first improvement is the change of training style of the whole framework. In BiGAN approach,
...
image approach used in the GAN framework.It tries to distinguish samples from a joint
...
\ref{sec:alad_alice}). F-AnoGAN framework addresses this issue by separating the training of
encoder network $E$ from the generator discriminator networks. New framework can be seen in Figure
