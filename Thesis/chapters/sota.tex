...
representation into the output data with the same dimensionality as the input. It is the composition of two operations.
Mapping from input to hidden layer is called encoding. Reconstructing data from the code using the mapping from hidden layer
to output layer is called decoding. However rather than trying to reconstruct the given data, autoencoders are mainly used 
to learn the representation about the data to extract meaningful features and to reduce the dimensionality of data while 
keeping the relevant information as an alternative to principal component analysis. 
...
With the increased complexity of the data presented, this architecture can be extended to capture more
...
The primal training objective for the autoencoders based on reconstruction loss. Depending on the
...
This reduces the functionality of the encoder-decoder networks to a identity function:
$$
...
the input is constantly noisy, autoencoder architecture gains a factor of robustness.
...
\subsection{AnoGAN}
\label{sec:anogan}
 AnoGAN model is first model that utilizes GANs for the anomaly detection task in our knowledge
...
then a predetermined cut-off value. This varies depending on the type of dataset and the dimensional
...
( see Section \ref{sec:bigan}). Generator module simultaneously learns the mapping from input data to a latent
...
\cite{cifar10} and SVHN \cite{Netzer2011ReadingDI} exceeds the previous mentioned models. This
